...
const { getPublicSite } = require("./sites");
...
// "Sites" is plain text, not a link — there's no /sites index page in
// Stage 1 to point it at.
function buildSiteBreadcrumbs(site) {
  return '<a href="/">Home</a> → <span>Sites</span> → <span>' + escapeHtml(site.domain) + "</span>";
}

function buildListingBreadcrumbs(result) {
  var parts = ['<a href="/">Home</a>', "→", "<span>Sites</span>"];
  if (result.site) {
    parts.push("→");
    parts.push(
