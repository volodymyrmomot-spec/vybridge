...
// untouched. Bounds are generous (not a strict [0,1]) since the anchor was
// chosen client-side specifically to fully contain the selection, so a
// well-formed submission should already sit close to that range — this
// only guards against a malformed/tampered request silently corrupting the
// live render, not against legitimate sub-pixel rounding.
function sanitizeAnchorData(body, viewportType) {
  ...
  const inRange = function (value) {
    return Number.isFinite(value) && value >= -0.05 && value <= 1.05;
  };
  if (!inRange(relativeX) || !inRange(relativeY)) {
    return null;
  }
  if (!Number.isFinite(relativeWidth) || relativeWidth <= 0 || relativeWidth > 1.05) {
    return null;
  }
  if (!Number.isFinite(relativeHeight) || relativeHeight <= 0 || relativeHeight > 1.05) {
    return null;
  }
