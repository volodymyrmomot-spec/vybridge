...
// The reference viewport each picker mode captures coordinates against.
// Never trusted from the client — finalizeSlot() derives
// pickerViewportWidth/Height server-side from the validated viewportType
// alone, so a manipulated request body can't inject a bogus reference size
// that would later corrupt w.js's mobile scale math.
const VIEWPORT_REFERENCE = {
  desktop: { width: 1440, height: 900 },
  mobile: { width: 400, height: 800 },
};

// The picker implementation version that produces slots finalized by the
// current code path — bumped only if a future picker generation changes
// how coordinates are captured, never reinterpreted after the fact.
const CURRENT_CAPTURE_VERSION = 1;
...
function publicSlot(slot) {
  return {
    ...
    anchorRelX: slot.anchorRelX,
    anchorRelY: slot.anchorRelY,
    anchorRelWidth: slot.anchorRelWidth,
    anchorRelHeight: slot.anchorRelHeight,
...
// Best-effort, entirely optional — a slot finalizes fine with none of this
// (falls back to posX/Y/Width/Height, exactly as before this existed).
// Only ever applied to desktop slots; mobile's positioning model is
// untouched.
//
// relativeX/relativeY bounds were originally too tight ([-0.05, 1.05]) —
// found via real-world diagnosis on Stavbahub: the anchor
// (findAnchorContainer() in w.js) is deliberately allowed to NOT fully
// enclose the selection when a narrower, width-capped container is
// preferred over a wider full-bleed one (see w.js's anchor-selection
// comment) — a selection that starts in the gutter beside such a container
// legitimately produces a relativeX/Y well outside [0,1] (e.g. -0.08 for a
// selection ~9% of the container's width into the gutter). [-0.5, 1.5]
// comfortably covers realistic gutter placements while still rejecting
// wildly out-of-range/tampered values. relativeWidth/relativeHeight only
// need to be finite and strictly positive, capped at 2 (a selection more
// than double the anchor's own size) as a sanity ceiling.
function sanitizeAnchorData(body, viewportType) {
  if (viewportType !== "desktop") {
    return null;
  }

  const anchorSelector = body && body.anchorSelector ? String(body.anchorSelector).trim().slice(0, 500) : "";
  const relativeX = body && body.relativeX !== undefined ? Number(body.relativeX) : NaN;
  const relativeY = body && body.relativeY !== undefined ? Number(body.relativeY) : NaN;
  const relativeWidth = body && body.relativeWidth !== undefined ? Number(body.relativeWidth) : NaN;
  const relativeHeight = body && body.relativeHeight !== undefined ? Number(body.relativeHeight) : NaN;
  ...
  if (!inRange(relativeX) || !inRange(relativeY)) {
    return null;
  }
  if (!Number.isFinite(relativeWidth) || relativeWidth <= 0 || relativeWidth > 2) {
    return null;
  }
  if (!Number.isFinite(relativeHeight) || relativeHeight <= 0 || relativeHeight > 2) {
    return null;
  }
...
function validateFinalizeSlot(body) {
  ...
  const viewportType = body && body.viewportType ? String(body.viewportType).trim() : "";
  ...
  if (viewportType !== "desktop" && viewportType !== "mobile") {
    errors.viewportType = "Choose a viewport (desktop or mobile) before creating the slot";
  }
...
async function finalizeSlot({ publisherId, slotId, body }) {
  ...
  // Derived server-side from the validated viewportType alone — never from
  // client-supplied numbers (see VIEWPORT_REFERENCE above).
  const viewportReference = VIEWPORT_REFERENCE[validated.viewportType];
  const anchorData = sanitizeAnchorData(body, validated.viewportType);
  ...
  const updateData = {
    ...
    status: status,
    viewportType: validated.viewportType,
    pickerViewportWidth: viewportReference.width,
    pickerViewportHeight: viewportReference.height,
    captureVersion: CURRENT_CAPTURE_VERSION,
    // domSelector is otherwise left untouched — still whatever placeholder
    // createPickerSession wrote, never consulted for positioning now that
    // posX/Y/Width/Height exist.
    // Set here, synchronously, so the finalize response already reflects
    // that capture is under way — flipped to ready/failed by
    // capturePreview() below, fired unawaited after this same update.
    previewStatus: "pending",
  };

  if (anchorData) {
    // Reused to hold the anchor container's CSS selector — see the field
    // comment on Slot.domSelector in schema.prisma.
    updateData.domSelector = anchorData.anchorSelector;
    updateData.anchorRelX = anchorData.relativeX;
    updateData.anchorRelY = anchorData.relativeY;
    updateData.anchorRelWidth = anchorData.relativeWidth;
    updateData.anchorRelHeight = anchorData.relativeHeight;
  }
  ...
  // Fire-and-forget — the finalize response above already reflects
  // previewStatus "pending" and doesn't wait on this. capturePreview()
  // updates the Slot itself once it finishes, whichever way.
  capturePreview(updated.id).catch(function (err) {
    console.error("[slots] Preview capture threw for slot " + updated.id + ":", err.message);
  });
...
async function deleteSlot({ publisherId, slotId }) {
  ...
  if (slot.previewImagePublicId) {
    deleteImage(slot.previewImagePublicId).catch(function (err) {
      console.error("[slots] Failed to clean up preview image for deleted slot " + slotId + ":", err.message);
    });
  }
