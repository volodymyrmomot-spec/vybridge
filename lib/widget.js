  ...
  for (const slot of slots) {
    ...
    if (deal && creativeItem) {
      result.push({
        ...
        // Only exposed when real anchor-relative capture data exists —
        // otherwise dom_selector is still just the finalize-time
        // placeholder, never a real anchor to resolve.
        anchor_selector: slot.anchorRelWidth != null ? slot.domSelector : null,
        anchor_rel_x: slot.anchorRelX,
        anchor_rel_y: slot.anchorRelY,
        anchor_rel_width: slot.anchorRelWidth,
        anchor_rel_height: slot.anchorRelHeight,
