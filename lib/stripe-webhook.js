...
const { notifyPendingApproval } = require("./deal-emails");
...
async function handlePaymentIntentSucceeded(tx, event) {
  ...
  const updated = await tx.deal.update({
    ...
    include: { publisher: true, slot: true },
  });
...
async function handleStripeWebhookRequest(req, res, sendJson) {
  ...
  // relations the pending_approval email needs) when it actually performed
  // the transition, so the notification only fires once per real event —
  // not on Stripe's occasional redelivery of an already-processed event.
  let dealEnteredPendingApproval = null;
  try {
    await prisma.$transaction(async function (tx) {
      ...
      if (event.type === "payment_intent.succeeded") {
        dealEnteredPendingApproval = await handlePaymentIntentSucceeded(tx, event);
  ...
  if (dealEnteredPendingApproval) {
    await notifyPendingApproval(dealEnteredPendingApproval);
  }
