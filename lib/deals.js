...
const { notifyApproved, notifyAutoApproved, notifyRejected } = require("./deal-emails");
...
// File type/size are already enforced by lib/multipart.js before this ever
// runs — this only covers what's specific to booking a slot.
function validateClickUrl(clickUrl) {
  if (!clickUrl || !/^https?:\/\//i.test(clickUrl)) {
    return "Creative clickUrl is required and must start with http:// or https://";
  }
  return null;
}
...
async function createDeal({ advertiserId, slotId, clickUrl, creativeFile }) {
  ...
  if (!creativeFile || !creativeFile.buffer || !creativeFile.buffer.length) {
  ...
  // Money always moves in PLATFORM_CURRENCY regardless of what a slot row
  // says (see lib/currency.js) — if a slot somehow got created in a
  // different currency, fail loudly here rather than silently charging the
  // advertiser the right number of cents in the wrong currency.
  if (slot.currency !== PLATFORM_CURRENCY) {
    return { status: 500, body: { ok: false, error: "Slot currency does not match the platform currency" } };
  }
  ...
  // Uploaded before the PaymentIntent is created — a failed upload should
  // never leave an unconfirmed PaymentIntent around needing cleanup.
  const uploaded = await uploadCreative({ buffer: creativeFile.buffer, mimeType: creativeFile.mimeType });

  const paymentIntent = await stripe.paymentIntents.create({
    amount: totalChargedCents,
    currency: PLATFORM_CURRENCY,
    customer: customerId,
    // Card-only checkout — no redirect-based payment methods, so
    // confirmation never needs a return_url.
        ...
        data: {
          ...
          creatives: {
            create: {
              ...
              fileSizeBytes: creativeFile.buffer.length,
              status: "submitted",
            },
          },
        },
  ...
  try {
    const updated = await prisma.$transaction(async (tx) => {
      ...
      return tx.deal.findUnique({
        where: { id: dealId },
        include: { advertiser: true, slot: { include: { site: true } } },
      });
    });

    if (actor === "system-auto-approve") {
      await notifyAutoApproved(updated);
    } else {
