...
const { validateChannels } = require("./blogger-channels");
const { generateUniqueSlug } = require("./slug");

const ROLES = ["advertiser", "publisher", "blogger"];
...
function validateRegisterFormat(body) {
  ...
  if (!ROLES.includes(role)) {
    errors.role = "Role must be advertiser, publisher, or blogger";
  }
  if (!country) {
  ...
  if (role === "blogger") {
    channelsResult = validateChannels(body && body.channels);
    Object.assign(errors, channelsResult.errors);
  }
  ...
  return {
    ...
    channels: channelsResult ? channelsResult.channels : [],
  };
}
...
async function createUser(body) {
  ...
  const user = await prisma.$transaction(async (tx) => {
    ...
    if (validated.role === "publisher") {
      const domain = extractHostname(validated.websiteUrl);
      // Checked against tx (not the outer prisma client) so the uniqueness
      // check and the insert below are part of the same transaction.
      const slug = await generateUniqueSlug(domain, async function (candidate) {
        const existing = await tx.site.findFirst({ where: { slug: candidate } });
        return !!existing;
      });
    ...
    if (validated.role === "blogger") {
      for (const channel of validated.channels) {
        await tx.bloggerChannel.create({
          data: {
            userId: created.id,
            ...
            openToNegotiation: channel.openToNegotiation,
          },
        });
      }
    }
