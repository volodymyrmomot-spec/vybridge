...
function validateRegisterBody(body) {
  ...
  if (!email) {
    ...
    errors.push("Email nie je platný");
  }
  if (!password) {
  ...
  if (!ROLES.includes(role)) {
    errors.push("Rola musí byť advertiser alebo publisher");
  }
  if (email && findUserByEmail(email)) {
    errors.push("Tento email je už zaregistrovaný");
  }
...
function authenticateUser(body) {
  ...
  if (!user || !verifyPassword(validated.password, user.passwordHash)) {
    return { status: 401, body: { ok: false, error: "Nesprávny email alebo heslo" } };
  }
