...
const { requestPasswordReset, resetPassword } = require("./password-reset");
...
async function handleAuthRequest(req, res, url, readBody, sendJson) {
  ...
  if (url.pathname === "/api/auth/forgot-password" && req.method === "POST") {
    const body = await readJsonBody(req, readBody);
    const result = await requestPasswordReset(body && body.email);
    sendJson(res, result.status, result.body);
    return true;
  }
  ...
  if (url.pathname === "/api/auth/reset-password" && req.method === "POST") {
    const body = await readJsonBody(req, readBody);
    const result = await resetPassword({ token: body && body.token, password: body && body.password });
    sendJson(res, result.status, result.body);
    return true;
  }
