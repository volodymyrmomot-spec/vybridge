...
async function getPublicSite(slug) {
  ...
  const slots = await prisma.slot.findMany({
    ...
    select: {
      ...
      previewPosX: true,
      previewPosY: true,
      previewPosWidth: true,
      previewPosHeight: true,
      pickerViewportWidth: true,
      pickerViewportHeight: true,
    },
  });
  ...
  return {
    ...
    listings: listings.map(function (listing) {
      // Two-hop aggregation (Listing.sourceId -> Slot), same pattern
      // getPublicSites() already uses — attaches the slot's Marketplace
      // preview screenshot and the position/reference-viewport fields the
      // frontend needs to draw the highlight overlay over it.
      const slot = slotById.get(listing.sourceId);
      return {
        slug: listing.slug,
        title: listing.title,
        ...
        // Opaque UUID, not itself sensitive (see lib/listings.js's
        // publicListing() for the same field at the per-listing level) —
        // needed by the "View placement on website" button to build the
        // ?slotPreview= link.
        sourceId: listing.sourceId,
        pageUrl: slot ? slot.pageUrl : null,
        previewImageUrl: slot ? slot.previewImageUrl : null,
        previewStatus: slot ? slot.previewStatus : null,
        posX: slot ? slot.posX : null,
        posY: slot ? slot.posY : null,
        posWidth: slot ? slot.posWidth : null,
        posHeight: slot ? slot.posHeight : null,
        // Resolved at screenshot-capture time (see resolveFinalRect() in
        // lib/slot-preview.js) — preferred by the frontend overlay over
        // posX/Y/Width/Height above, which can drift from what's actually
        // in the screenshot (anchor movement between Picker selection and
        // capture, or — for a no-anchor fixed-position slot — simply
        // being scroll-dependent live). Null until the first capture
        // succeeds.
        previewPosX: slot ? slot.previewPosX : null,
        previewPosY: slot ? slot.previewPosY : null,
        previewPosWidth: slot ? slot.previewPosWidth : null,
        previewPosHeight: slot ? slot.previewPosHeight : null,
        pickerViewportWidth: slot ? slot.pickerViewportWidth : null,
        pickerViewportHeight: slot ? slot.pickerViewportHeight : null,
      };
    }),
  };
}
...
async function getPublicSites() {
  ...
  const slots = await prisma.slot.findMany({
    where: { id: { in: slotIds } },
    select: { id: true, siteId: true, viewportType: true },
  });
  ...
  const slotIdToViewportType = new Map(
    slots.map(function (slot) {
      return [slot.id, slot.viewportType];
    })
  );
  ...
  activeListings.forEach(function (listing) {
    ...
    if (!bySite.has(siteId)) {
      bySite.set(siteId, { activeListingCount: 0, minPriceCents: null, formats: new Set(), viewports: new Set() });
    }
    ...
    const viewportType = slotIdToViewportType.get(listing.sourceId);
    if (viewportType) {
      agg.viewports.add(viewportType);
    }
  });
  ...
  return sites.map(function (site) {
    ...
    return {
      ...
      viewports: Array.from(agg.viewports),
    };
  });
}
