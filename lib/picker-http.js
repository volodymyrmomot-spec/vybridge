...
const { createPickerSession, createPickerToken, submitSelector } = require("./picker");
...
async function handlePickerRequest(req, res, url, readBody, sendJson) {
  if (url.pathname === SESSION_ROUTE && req.method === "POST") {
    ...
    const result = await createPickerSession({ publisherId: user.id, pageUrl: body.pageUrl });
