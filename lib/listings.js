...
async function getPublicListing(slug) {
  ...
  return {
    ...
    slot: {
      ...
      // See the same field on lib/sites.js's getPublicSite() — resolved at
      // capture time, preferred over posX/Y/Width/Height by the overlay.
      previewPosX: slot.previewPosX,
      previewPosY: slot.previewPosY,
      previewPosWidth: slot.previewPosWidth,
      previewPosHeight: slot.previewPosHeight,
      pickerViewportWidth: slot.pickerViewportWidth,
      pickerViewportHeight: slot.pickerViewportHeight,
    },
  };
}
